/**
 * Shared UI primitives: Toggle, Slider, Tabs, Stars, Difficulty, etc.
 */
import React from 'react';
import I from '../Icons';

export function Stars({ value, max = 5, size = 12, gold = true, onChange }) {
    return (
                ...
                return (
                    ...
                    >
                        <svg width={size} height={size} viewBox="0 0 20 20" fill="currentColor">
                            <path d="M10 1.5l2.6 5.2 5.7.8-4.1 4 1 5.7L10 14.6 4.8 17.2l.9-5L2.8 8.2l5-.6L10 1.5z" />
                        </svg>
                    </span>
                );
            })}
...
export function Difficulty({ value, max = 5 }) {
    return (
...
export function Avatar({ initial, size = 28, gold = false }) {
    return (
        <div style={{
...
export function Slider({ value, min = 0, max = 100, step = 1, onChange, label, suffix }) {
...
export function Modal({ open, onClose, children, width = 720, padding = 0 }) {
    if (!open) return null;
    return (
...
const UI = { Stars, Difficulty, Toggle, Avatar, Slider, Tabs, Modal, SectionTitle, Pill };
export default UI;
