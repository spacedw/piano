...
export default function Piano({ activeNotes = new Map(), songActiveNotes = [], width = 1200, height = 160 }) {
    ...
    // Determine active set (combines user MIDI input + song playback notes)
    const activeSet = useMemo(() => {
        const set = new Map();
        // User played notes (gold accent)
        activeNotes.forEach((velocity, midi) => {
            set.set(midi, { velocity, source: 'user' });
        });
        // Song playback notes (if not user-played, show as song)
